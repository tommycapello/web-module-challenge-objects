...
createMenuItem("spaghetti", 10, 'dinner');
...
export const burger = {
  ...
  category: "Lunch",
  discount(jobTitle){
    if(jobTitle==='teacher' || jobTitle==='student'){
      let total = (burger.price * .75);
      return total;
    }else{
      let total = (burger.price * .90);
      return total;
    }
  }
}
...
*/
reviews.push({name:"Tommy", rating:2, feedback: "I had a terrible time."});
...
function getLastReview(array2) {
return `${array2[array2.length-1].name} gave the restaurant a ${array2[array2.length-1].rating} star review, and their feedback was: ${array2[array2.length-1].feedback}`;
};
